...
function buildNarrativeRole(
  ...
  if (signals.policyAccess) {
    return `This post extends BeOne's story from science to systems — it argues that innovation only matters if patients can access it, bridging ${beat} content to policy advocacy. This positioning is rare in pharma social and attracts shares from patient advocacy groups and healthcare policy influencers who don't typically engage with product content. → Build a 3-post access arc: (1) name the barrier, (2) show BeOne's response, (3) feature a patient whose access improved. The arc format turns a one-time policy post into a sustained narrative thread.`;
  }

  if (signals.awarenessDay) {
    return `This post participates in a shared cultural moment to keep BeOne visible in disease communities without leading with product claims — a credibility move that often outperforms direct brand posts in organic reach. → Plan 30 days out: the most effective awareness day posts incorporate patient voices, purposeful visuals, and a clear action (a resource link, a hashtag) rather than a last-minute text-only post. Set a calendar reminder now for the next relevant awareness date.`;
  }

  if (signals.teamCulture) {
    return `This post reinforces the People & Culture dimension of BeOne's arc — it shows who carries the mission, not just what the company ships. Culture content signals organizational health to investors and prospective employees simultaneously. → Go deeper in the org: profile team members across different functions (R&D, patient access, regulatory, clinical ops) rather than concentrating on leadership. Cross-functional culture content reaches a wider internal-referral network when employees share it.`;
  }

  if (ctx.chronologicalRank === 0) {
    return `This is the most recent expression of ${beat} on ${post.platform}, setting the current tone for everything that follows — it defines the "state of the narrative" for anyone landing on the profile today. → Audit what this post says first: consider whether this is the message BeOne most wants leading the channel right now. If a higher-priority beat (e.g., a clinical milestone) should greet new visitors, plan the next post accordingly.`;
  }
