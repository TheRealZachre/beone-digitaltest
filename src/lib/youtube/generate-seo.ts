...
import type { SeoPackage, VideoAnalyzeRequest, VideoAnalyzeResponse } from "./types";
...
function buildDescription(
  ...
  return [
    ...
    "🔗 Learn more in the description links",
    "",
    `#${keywords[0] ?? "video"} #${keywords[1] ?? "education"} #youtube`,
  ].join("\n");
}
...
export function analyzeUploadedVideo(
  ...
  return {
    ...
    transcript,
    transcriptionSource:
      options.transcriptionSource ?? ("manual" as const),
    current,
    generated,
    projected,
    keywords,
