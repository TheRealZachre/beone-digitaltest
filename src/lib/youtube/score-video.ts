...
function scoreRetention(video: Pick<YouTubeVideo, "viewCount" | "durationSeconds" | "publishedAt">): number {
...
function scoreEngagement(video: Pick<YouTubeVideo, "viewCount" | "likeCount" | "commentCount">): number {
...
function buildRecommendations(
  ...
  if (scores.title < 70) {
    if (video.title.length > 70) {
      recs.push("Shorten title to under 70 characters for full search visibility.");
    } else {
      recs.push("Add primary keywords earlier in the title to improve click appeal.");
    }
  }
  ...
  if (scores.tags < 70) {
    recs.push("Add more relevant tags aligned with title and transcript keywords.");
  }
  if (scores.thumbnail < 70) {
    recs.push("Use a high-resolution custom thumbnail with bold, readable text.");
  }
  if (scores.engagement < 60) {
    recs.push("Add an end-screen CTA to boost likes, comments, and subscriptions.");
  }
